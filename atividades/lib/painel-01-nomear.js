/* =============================================================================
   MÓDULO DE PAINEL — Rodada 1 "Nomear o fenômeno"
   ...
   de debate ("8 disseram hipotimia, 3 anedonia — qual a fronteira?").
   ...
   Metadados dos itens vêm de window.NOMEAR_DATA (mesma fonte da atividade).
...
(function (global) {
  ...
  function injetarCSS(){
    if (document.getElementById("css-painel-nomear")) return;
    var s = document.createElement("style");
    s.id = "css-painel-nomear";
    s.textContent =
  ...
  function render(container, grupos /*, cores */){
    var ITENS = global.NOMEAR_DATA.ITENS;
  ...
  global.registrarPainel({
    id: "nomear",                   // === valor da coluna `atividade`
    label: "Nomear o fenômeno",     // rótulo da aba / instrução do código
    rota: "01-nomear.html",         // página da atividade
    placar: true,                   // pontua → placar genérico do shell
    maxPontos: global.NOMEAR_DATA.ITENS.length, // 10 itens × 1 pt (normaliza /100)
    itensParaCompletar: global.NOMEAR_DATA.ITENS.length, // aba "concluída" = todos fizeram os itens
    dataGlobal: "NOMEAR_DATA",      // window[...] carregado via <script> antes daqui
    render: render
  });
})(window);
